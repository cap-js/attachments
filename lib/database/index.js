...
class DBAttachmentsService extends AttachmentsService {
    ...
    async onPUT(source, target, items) {
        ...
        // for x in properties (@type Image or Attachments)
        if (!hasData) {
            items.forEach(item => {
                data.push({
                    entityKey: item.ID,
                    attachments: [{
                        fileName: item.logs_fileName
                    }]
                },
                {
                    entityKey: item.ID,
    ...
    async onGET(entity) {
        // TODO: How to determine property names 'logs' and 'screenshot'
        // for x in properties (@type Image or Attachments)
        return (await SELECT.from(entity)).filter(item => item.logs_fileName || item.screenshot_fileName)
    }

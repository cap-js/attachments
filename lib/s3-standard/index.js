...
class AWSAttachmentsService extends AttachmentsService {
    ...
    async onGET(entity, whereCondition) {
        ...
        const command = whereCondition ?
            new ListObjectsCommand({ Bucket: bucket, Prefix: `sap.capire.incidents.Customers-${whereCondition.ID}` }) :
    ...
    async onSTREAM(entity, ID) {
        const command = new GetObjectCommand({
            ...
            Key: `sap.capire.incidents.Customers-${ID}.png`
        })
        const res = await this.client.send(command)

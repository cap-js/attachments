...
module.exports = class AzureAttachmentsService extends require("./basic") {
  ...
  async get(attachments, keys, req = {}) {
    ...
    logConfig.processStep('Starting file download from Azure Blob Storage', {
      attachmentEntity: attachments.name,
      keys,
      tenant: req?.tenant
    })
    ...
    try {
      ...
      logConfig.debug('Downloading file from Azure Blob Storage', {
        containerName: this.containerName,
        blobName
      })
      ...
      const duration = Date.now() - startTime
      logConfig.debug('File download from Azure Blob Storage completed successfully', {
        fileId: keys.ID,
        containerName: this.containerName,
        blobName,
        duration
  ...
  async updateContentHandler(req, next) {
    logConfig.debug(`[Azure Upload] Uploading file using updateContentHandler for ${req.target.name}`)
    // Check separate object store instances
    if (separateObjectStore) {
    ...
    if (req?.data?.content) {
      ...
      if (response?.url) {
        ...
        logConfig.debug(`[Azure Upload] Uploaded file using updateContentHandler for ${req.target.name}`)
      }
    } else if (req?.data?.note) {
      const key = { ID: targetID }
      await super.update(req.target, key, { note: req.data.note })
      logConfig.debug(`[Azure Upload] Updated file upload with note for ${req.target.name}`)
    } else {
      next()
    }
  }
  ...
  async delete(blobName, req) {
    logConfig.debug(`[Azure Upload] Executing delete for file in ${req.target.name}`)
    // Check separate object store instances
    if (separateObjectStore) {
      const tenantID = cds.context.tenant
      await this.createAzureClient(tenantID)

...
module.exports = class AzureAttachmentsService extends require("./object-store") {
  ...
  /**
   * Returns tenant-specific or shared container client, depending on whether separate object stores are configured, 
   * for communicating to the BTP Object store on Azure landscapes.
   * @returns {Promise<import('@azure/storage-blob').ContainerClient>}
   */
  async getContainerClient() {
      const cacheKey = this.separateObjectStore ? cds.context.tenant : 'shared'
      const existingClient = this.clientsCache.get(cacheKey);
      if (existingClient) {
        return existingClient.containerClient
      } else {
        return (await this.createClient(cacheKey)).containerClient;
      }
    }
  ...
  async createClient(tenantID) {
    LOG.info('Creating tenant-specific Azure Blob Storage client', { tenantID })
  ...
  */
  async put(attachments, data, isDraftEnabled, _content, req) {
    ...
    LOG.info('Starting file upload to Azure Blob Storage', {
  ...
  */
  async get(attachments, keys) {
    ...
    LOG.info('Starting stream from Azure Blob Storage', {
      attachmentEntity: attachments.name,
      keys,
      tenant: tenantID
    })
  ...
  async updateContentHandler(req, next) {
    LOG.debug(`[Azure] Uploading file using updateContentHandler for ${req.target.name}`)
    ...
    if (req?.data?.content) {
      ...
      if (response?.url) {
        ...
        LOG.debug(`[Azure] Uploaded file using updateContentHandler for ${req.target.name}`)
      }
    } else if (req?.data?.note) {
      const key = { ID: targetID }
      await super.update(req.target, key, { note: req.data.note })
      LOG.debug(`[Azure] Updated file upload with note for ${req.target.name}`)
    } else {
      next()
    }
  }

...
module.exports = class GoogleAttachmentsService extends require("./basic") {
    ...
    if (!separateObjectStore) {
      ...
      if (!creds) {
        if (Object.keys(creds).includes('bucket')) {
          throw new Error('AWS S3 or Azure credentials found where Google Cloud Platform credentials expected, please check your service bindings.')
        }
        throw new Error("SAP Object Store instance is not bound.")
      }

      // Validate required credentials
      const requiredFields = ['bucket_name', 'project_id', 'service_account']
      ...
      logConfig.info('Configuring shared Google Cloud Platform client', {
        ...
        hasServiceAccount: !!creds.service_account
      })

      this.bucketName = creds.bucket_name
      this.storageClient = new Storage({
        projectId: creds.project_id,
        credentials: creds.service_account // or path to key file
      })
      this.bucket = this.storageClient.bucket(creds.bucket_name)
  ...
  async createGoogleClient(tenantID) {
    ...
    try {
      ...
      const requiredOsFields = ['bucket_name', 'project_id', 'service_account']
      ...
      logConfig.debug('Creating Google Cloud Platform client for tenant', {
        tenantID,
        bucketName: objectStoreCreds.credentials.bucket_name
      })
  ...
  */
  ...
  async updateContentHandler(req, next) {
    logConfig.debug(`[Google] Uploading file using updateContentHandler for ${req.target.name}`)
    // Check separate object store instances
    if (separateObjectStore) {
      const tenantID = cds.context.tenant
      await this.createGoogleClient(tenantID)
    ...
    if (req?.data?.content) {
      ...
      if (response?.url) {
        ...
        logConfig.debug(`[Google] Uploaded file using updateContentHandler for ${req.target.name}`)
      }
    } else if (req?.data?.note) {
      const key = { ID: targetID }
      await super.update(req.target, key, { note: req.data.note })
      logConfig.debug(`[Google] Updated file upload with note for ${req.target.name}`)
    } else {
      next()
    }
  }
  ...
  async delete(blobName) {
    const tenantID = cds.context.tenant
    logConfig.debug(`[Google] Executing delete for file ${blobName} in bucket ${this.bucketName}`)

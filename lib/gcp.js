...
module.exports = class GoogleAttachmentsService extends require("./object-store") {
  ...
  /**
   * Returns tenant-specific or shared bucket, depending on whether separate object stores are configured, 
   * for communicating to the BTP Object store on GCP landscapes.
  ...
  async getBucket() {
    ...
    if (existingClient) {
      ...
      return (await this.createClient(cacheKey)).bucket;
    }
  }
  ...
  async createClient(tenantID) {
    LOG.info('Creating tenant-specific Google Cloud Platform client', { tenantID })
  ...
  */
  async put(attachments, data, isDraftEnabled, _content, req) {
    ...
    LOG.info('Starting file upload to Google Cloud Platform', {
  ...
  */
  ...
  async updateContentHandler(req, next) {
    LOG.debug(`[GCP] Uploading file using updateContentHandler for ${req.target.name}`)
    ...
    if (req?.data?.content) {
      ...
      if (response?.url) {
        ...
        LOG.debug(`[GCP] Uploaded file using updateContentHandler for ${req.target.name}`)
      }
    } else if (req?.data?.note) {
      const key = { ID: targetID }
      await super.update(req.target, key, { note: req.data.note })
      LOG.debug(`[GCP] Updated file upload with note for ${req.target.name}`)
    } else {
      next()
    }
  }

const fs = require('fs')
const { connectToAttachmentsService, getContentType, getElementsOfType } = require('./helpers')
...
const ReadHandler = async (req, next) => {
    ...
    if (imageIsRequested) {
      ...
      const { fileName } = await SELECT.one.from('sap.attachments.Images').columns('fileName')
    ...
    if (attachmentIsRequested && AttachmentsSrv.name !== 'DBAttachmentsService') {
      const { ID } = req.data
      const { fileName } = await SELECT.one.from('sap.attachments.Attachments').where({ ID })
      const ext = fileName.split('.').pop()
      const stream = await AttachmentsSrv.onSTREAM(req.entity, ext, ID)
      return {
        value: stream

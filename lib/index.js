const { connectToAttachmentsService, getAssociationsForType } = require('./helpers')
...
const ReadImagesHandler = async (req, next) => {
    ...
    const ImagesFromAssocs = [['customer', 'avatar']] //getAssociationsForType(req, 'Image')
...
const ReadAttachmentsHandler = async (req, next) => {
    ...
    if (data.length === 0) {
        ...
        let res = await media_srv.onGET(`${srv}.AttachmentsView`, { entityKey: ID })
        console.log(res)
        for (const r of res) {

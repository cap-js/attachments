...
module.exports = class AWSAttachmentsService extends require("./object-store") {
  ...
  /**
   * Returns tenant-specific or shared client, depending on whether separate object stores are configured, 
   * for communicating to the BTP Object store on AWS landscapes.
  ...
  async getClient() {
    ...
    if (existingClient) {
      ...
      return (await this.createClient(cacheKey)).client;
    }
  }
  ...
  async createClient(tenantID) {
    LOG.debug('Creating S3 client for', { tenantID })

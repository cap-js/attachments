...
module.exports = class AWSAttachmentsService extends require("./basic") {
  ...
  async put(attachments, data, isDraftEnabled, _content, req) {
    ...
    try {
      ...
      if (this.kind === 's3') {
        ...
        const scanBackgroundJob = cds.spawn(async () =>
          await scanRequest(attachments, { ID: metadata.ID }, req)
        );

        scanBackgroundJob.on('error', err => {
          logConfig.withSuggestion('error',
  ...
  async attachDeletionData(req) {
    const attachments = cds.model.definitions[req?.target?.name + ".attachments"] || cds.model.definitions[req?.target?.name + "_attachments"]
    if (attachments) {

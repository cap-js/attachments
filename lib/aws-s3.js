...
const isMultitenant = !!cds.env.requires.multitenancy;
const objectStoreKind = cds.env.requires.attachments.objectStore.kind;
const separateObjectStore = isMultitenant && objectStoreKind === "separate";
...
module.exports = class AWSAttachmentsService extends require("./basic") {
  init() {
    ...
    if (!separateObjectStore) {
      ...
      if (!creds) console.error("SAP Object Store instance is not bound.");
  ...
  async createClientS3(tenantID) {
    try {
      ...
      if (!serviceManagerCreds) {
        // eslint-disable-next-line no-console
        console.error("Service Manager Instance is not bound");
      }
      ...
      if (!objectStoreCreds) {
        // eslint-disable-next-line no-console
        console.error(`SAP Object Store instance not bound for tenant ${tenantID}`);
      }
  ...
  async put(attachments, data, _content, req) {
    // Check for multitenancy and separate object store instances
    if (separateObjectStore) {

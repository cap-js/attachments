...
const profile_multitenacy = true;
...
module.exports = class AWSAttachmentsService extends require("./basic") {
  ...
  async createClientS3(tenantID) {
    console.log("Inside createClientS3", tenantID);
    console.log("checking the profile")
    console.log("Profile inside createclientS3", profile_multitenacy);

    if (profile_multitenacy) {
      try {
        console.log("mtx-sidecar profile it is")
        // Check if the tenant's S3Client is already cached
        if (s3ClientsCache[tenantID]) {
          return s3ClientsCache[tenantID];
        }
        
        console.log("will satrt fetching the service manager creds")
        console.log("cds env vcap", cds.env.requires.serviceManager)
        console.log("consoling the response from service manager api",cds.env.requires.serviceManager.credentials)
        ...
        console.log("clien_secret", clientsecret)
        ...
        console.log("error cretaing client bucket S3",error)
      }
    } else {

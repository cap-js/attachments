...
module.exports = class AWSAttachmentsService extends require("./basic") {
  ...
  async initialDataUpload(Attachments, data, _content) {
    if (Array.isArray(data))
      return Promise.all(data.map((d) => this.initialDataUpload(Attachments, d)));
  ...
  async DeleteAttachmentsAfterDelete(records, req) {
  ...
  async AttachmentDataForDelete(req) {
    ...
    if(attachments){
       let attachmentsToDelete = await SELECT.from(attachments).columns("url").where({up__ID: req.data.ID});
  ...
  registerUpdateHandlers(srv, entity, mediaElement) {
    srv.before("DELETE", entity, this.AttachmentDataForDelete.bind(this));

...
module.exports = class AWSAttachmentsService extends require("./basic") {
  ...
  async createClientS3(tenantID) {
    ...
    try {
      ...
      logConfig.s3Operation('client_creation', tenantID, true, {
        bucket: this.bucket,
        region: objectStoreCreds.credentials.region
      });

    } catch (error) {
      logConfig.s3Operation('client_creation', tenantID, false, {
        errorMessage: error.message,
        suggestion: 'Check Service Manager and Object Store instance configuration'
      });
      throw error;
    }
  }
  ...
  async put(attachments, data, isDraftEnabled, _content, req) {
    ...
    try {
      ...
      logConfig.fileOperation('upload_start', metadata.filename, metadata.ID, true, {
        bucket: this.bucket,
        key: Key,
      ...
      logConfig.fileOperation('upload_complete', metadata.filename, metadata.ID, true, {
        bucket: this.bucket,
        key: Key,
        duration
      });
      ...
      if (this.kind === 's3') {
        ...
        scanRequest(attachments, { ID: metadata.ID }, req);
      }

    } catch (err) {
      const duration = Date.now() - startTime;
      logConfig.fileOperation('upload_failed', data?.filename, data?.ID, false, {
        bucket: this.bucket,
        duration,
        errorMessage: err.message,
        suggestion: 'Check S3 credentials, bucket permissions, and network connectivity'
      });
      throw err;
    }
  }
  ...
  async get(attachments, keys, req = {}) {
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      logConfig.fileOperation('download_complete', 'unknown', keys.ID, true, {
        bucket: this.bucket,
        key: Key,
        duration,
      ...
      logConfig.fileOperation('download_failed', 'unknown', keys?.ID, false, {
        bucket: this.bucket,
        duration,

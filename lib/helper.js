...
function buildFilenameCondition(incomingFilenames) {
  ...
  for (const filename of incomingFilenames) {
    ...
    xpr.push(
      "(",
      { ref: ["filename"] }, "=", { val: filename },
      "or",
      { ref: ["filename"] }, "like", { val: `${base}-%${ext}` },
      ")",
    )
  }
  return { xpr }
...
function buildSuffixOrderBy(dbKind) {
  if (dbKind === "hana") {
    return `LPAD(SUBSTRING_REGEXPR('([0-9]+)(?!.*[0-9])' IN filename), 10, '0')`
  } else if (dbKind === "postgres") {
    return `CAST(REGEXP_REPLACE(filename, '.*-(\\d+)\\..*', '\\1') AS INTEGER)`
  } else if (dbKind === "sqlite") {
    return `CAST(SUBSTR(filename, INSTR(filename, '-') + 1, INSTR(SUBSTR(filename, INSTR(filename, '-') + 1), '.') - 1) AS INTEGER)`
  }
  return null
}
...
async function handleDuplicates(req, entries, attachmentTarget, parentKeys) {
  ...
  const allExistingAttachments = await db.run(query)

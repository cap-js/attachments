...
function buildSuffixOrderBy(dbKind) {
  if (dbKind === "hana") {
    return cds.ql.expr`LPAD(SUBSTRING_REGEXPR('([0-9]+)(?!.*[0-9])' IN filename), 10, '0')`
  } else if (dbKind === "postgres") {
    return cds.ql.expr`CAST(REGEXP_REPLACE(filename, '.*-(\\d+)\\..*', '\\1') AS cds.Integer)`
  } else if (dbKind === "sqlite") {
    return cds.ql.expr`CAST(SUBSTR(filename, INSTR(filename, '-') + 1, INSTR(SUBSTR(filename, INSTR(filename, '-') + 1), '.') - 1) AS cds.Integer)`
  }
  return null
}

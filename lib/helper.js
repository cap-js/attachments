...
function validateServiceManagerCredentials(serviceManagerCreds) {
  ...
  if (missingSmFields.length > 0) {
    LOG.error(
      ...
      cds.utils.redacted(serviceManagerCreds),
...
/**
 * Resolves the effective attachment storage kind based on CDS environment and object store credentials.
 * Returns a specific provider ("aws-s3", "azure-blob-storage", "gcp") when kind is "standard",
 * otherwise returns "db".
 * @returns {string} The resolved attachment kind
 */
 ...
 * @param {import('stream').Readable} input - The readable stream to hash
...
/**
 * Scans request entries to collect the set of parent entity keys and incoming filenames.
 * Handles both deep-insert (entries with a composition array) and single-attachment entries.
 * @param {object[]} entries - The request data entries to scan
 * @param {string[]} parentKeys - The up__-prefixed parent key field names
 * @param {string|null} compositionFieldName - The composition field name for deep inserts, or null
 * @returns {{ parentTupleMap: Map<string, object>, incomingFilenames: Set<string> }}
 */
function collectParentAndFilenames(entries, parentKeys, compositionFieldName) {
  const parentTupleMap = new Map()
  const incomingFilenames = new Set()

  for (const entry of entries) {
    const compField =
      compositionFieldName && Array.isArray(entry[compositionFieldName])
    ...
    if (compField) {
      ...
      for (const att of entry[compField]) {
        if (att.filename) incomingFilenames.add(att.filename)
      }
  ...
  return { parentTupleMap, incomingFilenames }
}

/**
 * Applies filename deduplication to all attachments in the request entries,
 * using the pre-built map of existing filenames per parent key.
 * @param {object[]} entries - The request data entries to mutate
 * @param {string[]} parentKeys - The up__-prefixed parent key field names
 * @param {string|null} compositionFieldName - The composition field name for deep inserts, or null
 * @param {Object.<string, Set<string>>} existingFilenamesByParent - Map of compound parent key to existing filenames
 */
function applyRenames(
  entries,
  parentKeys,
  compositionFieldName,
  existingFilenamesByParent,
) {
  for (const entry of entries) {
    const compField =
      compositionFieldName && Array.isArray(entry[compositionFieldName])
    ...
    if (compField) {
      ...
      for (const attachment of entry[compField]) {
 ...
 * @param {string} compositionFieldName - The name of the composition field for attachments.
 */
async function handleDuplicates(
  ...
  const { parentTupleMap, incomingFilenames } = collectParentAndFilenames(
    entries,
    parentKeys,
    compositionFieldName,
  )

  if (parentTupleMap.size === 0 || incomingFilenames.size === 0) return
...
/**
 * Walks a composition path through a CDS entity's elements to resolve the leaf target entity.
 * @param {import('@sap/cds').entity} root - The starting entity definition
 * @param {string[]} path - Array of element names to traverse (e.g. ["attachments"] or ["details", "attachments"])
 * @returns {import('@sap/cds').entity|undefined} The resolved target entity, or undefined if the path is invalid
 */

...
async function fetchObjectStoreBinding(tenantID, token) {
  ...
  const response = await _fetch(
    `${sm_url}/v1/service_bindings?${params}`,
    {
      headers: {
 ...
 * @param {string} [attachmentFieldName] - The name of the attachments composition field on the parent entity.
 */
async function handleDuplicates(
  ...
  for (const entry of entries) {
    if (
      attachmentFieldName &&
      entry[attachmentFieldName] &&
      Array.isArray(entry[attachmentFieldName])
    ) {
      // Deep insert case
      ...
      for (const att of entry[attachmentFieldName]) {
        if (att.filename) {
  ...
  for (const entry of entries) {
    if (attachmentFieldName && entry[attachmentFieldName]) {
      ...
      for (const attachment of entry[attachmentFieldName]) {

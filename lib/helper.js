...
async function handleDuplicates(req, entries, attachmentTarget, parentKey) {
  ...
  if (db.kind === "hana") {
    dbParentKey = dbParentKey.toUpperCase()
    fileNameKey = fileNameKey.toUpperCase()
    query = `
        SELECT * FROM "${tableName.toUpperCase().replace(/"/g, '""')}"
        WHERE "${dbParentKey}" IN (${[...parentIDs].map(() => "?").join(",")})
    ...
    query = `
        ...
        WHERE "${dbParentKey}" IN (${[...parentIDs].map((_, i) => `$${i + 1}`).join(",")})
          AND (${[...incomingFilenames]
            .map((filename) => {
              const base = basename(filename, extname(filename)).replace(
                /'/g,
  ...
  const existingFilenamesByParent = allExistingAttachments.reduce(
    (acc, att) => {
      const parentID = att[dbParentKey]
      if (!acc[parentID]) {
        acc[parentID] = new Set()
      }
      acc[parentID].add(att[fileNameKey])
      return acc
    },
    {},
  )

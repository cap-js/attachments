 ...
 */
async function handleDuplicateFilename(data, target) {
  if (!data.filename) {
    return
  }
  ...
  const existingFilenames = allAttachments.map((att) => att.filename)

  if (existingFilenames.includes(data.filename)) {
    const { extname } = require("path")
    const ext = extname(data.filename)
    const basename = data.filename.slice(0, -ext.length)
    let counter = 1
    let newFilename

...
function sizeInBytes(size, target) {
  if (!size || (typeof size !== "string" && typeof size !== "number")) {
    LOG.warn(
      `Could not determine the maximum byte size for the content of ${target}`,
    )
    return
  }
...
/**
 * Handles duplicate filenames by appending a suffix right before insertion.
 * @param {object} data - The attachment data.
 * @param {import('@sap/cds').Entity} target - The target attachment entity.
 * @param {Set<string>} existingFilenamesSet - A set of existing filenames to check against.
 */
async function handleDuplicateFilename(data, target, existingFilenamesSet) {
  if (!data.filename) {
    return
  }

  let existingFilenames = existingFilenamesSet
  if (!existingFilenames) {
    const parentKeys = Object.keys(data).filter((key) => key.startsWith("up__"))
    ...
    const allAttachments = await SELECT.from(target).where(whereClause)
    existingFilenames = new Set(
      allAttachments.map((att) => att.filename).filter(Boolean),
    )
  }

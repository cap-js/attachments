...
async function fetchTokenWithMTLS(certURL, clientid, certificate, key) {
  ...
  try {
    ...
    const duration = Date.now() - startTime;
    const suggestion = error.code === 'CERT_REQUIRED' ?
      'Check if MTLS certificate and key are properly formatted' :
      error.response?.status === 401 ?
        'Verify MTLS certificate is valid and authorized for this client' :
        error.response?.status === 404 ?
          'Check certificate URL is correct' :
          'Verify certificate authority and SSL configuration';

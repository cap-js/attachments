 ...
 */
async function handleDuplicates(req, entries, attachmentTarget, parentKeys) {
  const { extname, basename } = require("path")
  if (!Array.isArray(parentKeys)) parentKeys = [parentKeys]

  // Build a single string key from an ordered list of values for Map lookups
  const makeTupleKey = (values) => values.map(String).join("\x00")
  ...
  let dbParentKeys = [...parentKeys]

  // Single key  → WHERE "up__ID" IN (?, ?)
  // Composite   → WHERE ("up__sampleID" = ? AND "up__gjahr" = ?) OR (...)
  const buildParentWhere = (dialect) => {
    if (dbParentKeys.length === 1) {
      const col = dbParentKeys[0]
      const placeholders = tuples
        .map((_, i) => (dialect === "postgres" ? `$${i + 1}` : "?"))
        .join(", ")
      return {
        sql: `"${col}" IN (${placeholders})`,
        params: tuples.map((t) => t[parentKeys[0]]),
      }
    }
    let paramIdx = 1
    const conditions = tuples.map((_) => {
      const clauses = dbParentKeys.map((k) =>
        dialect === "postgres" ? `"${k}" = $${paramIdx++}` : `"${k}" = ?`,
      )
      return `(${clauses.join(" AND ")})`
    })
    return {
      sql: conditions.join(" OR "),
      params: tuples.flatMap((t) => parentKeys.map((k) => t[k])),
    }
  }
  ...
  if (db.kind === "hana") {
    dbParentKeys = parentKeys.map((k) => k.toUpperCase())
    fileNameKey = "FILENAME"
    const { sql: parentWhere, params } = buildParentWhere("hana")
    queryParams = params
    query = `
        SELECT * FROM "${tableName.toUpperCase().replace(/"/g, '""')}"
        WHERE ${parentWhere}
    ...
    const { sql: parentWhere, params } = buildParentWhere("postgres")
    queryParams = params
    query = `
    ...
    const { sql: parentWhere, params } = buildParentWhere("sqlite")
    queryParams = params
    query = `
        SELECT * FROM "${tableName.replace(/"/g, '""')}"
        WHERE ${parentWhere}
    ...
    req.error({
      code: 501,
      message: `Unsupported database for optimized suffix handling: '${db.kind}'. Please implement the required raw SQL query.`,
    })
    return
  }

  const allExistingAttachments = await db.run(query, queryParams)

  // Group existing filenames by compound parent key
  const existingFilenamesByParent = allExistingAttachments.reduce(
    (acc, att) => {
      const compoundKey = makeTupleKey(dbParentKeys.map((k) => att[k]))
      if (!acc[compoundKey]) acc[compoundKey] = new Set()
      acc[compoundKey].add(att[fileNameKey])
      return acc
    },
    {},
  )

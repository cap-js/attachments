...
// Register the db.before and db.after handlers ONCE (not per-service)
cds.once("served", async () => {
  if (!cds.env.requires.attachments) return
  const db = await cds.connect.to("db")

  db.before("CREATE", async function handleDBBefore(req) {
    if (
      !req.data ||
    ...
    // Store processed IDs for the db.after handler to trigger malware scan
    if (processedIds.length > 0) {
      req.handledAttachments = { target: req.target, ids: processedIds }
    }
  })

  db.after("CREATE", async function handleDBAfter(data, req) {
    if (!req.handledAttachments) return
    const handled = req.handledAttachments
    if (!handled) return

    delete req.handledAttachments

    LOG.debug("Triggering malware scan after DB INSERT", {
      attachmentEntity: handled.target.name,
      ids: handled.ids,
    })

    const MalwareScanner = await cds.connect.to("malwareScanner")
    await Promise.all(
      handled.ids.map(async (id) => {
        await MalwareScanner.emit("ScanAttachmentsFile", {
          target: handled.target.name,
          keys: { ID: id },
        })
      }),
    )

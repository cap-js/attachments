...
cds.ApplicationService.handle_attachments = cds.service.impl(async function () {
    ...
    this.on(
      ...
      async function createAttachments(req, next) {
        if (!req.target._attachments.isAttachmentsEntity ||
          ...
          !req.data.url ||
          !(req.data.content ||
            (Array.isArray(req.data) && req.data[0]?.content))
        ) return next()
  ...
  /**
   * Applies message keys with target and field name if available
   * @param {string} message - base message key
   * @param {string} targetName - target entity name
   * @param {string} fieldName - field name
   * @returns {string} - modified message key
   */
  ...
  /**
   * Recursively retrieves columns to check compositions for min/max items
   * @param {import('@sap/cds').Entity} target - target entity
   * @returns {Array} - columns to select
   */
  function retrieveToCheckCompositions(target) {
    ...
    for (const compName in target.compositions) {
      ...
      if (comp['@Validation.MaxItems'] || comp['@Validation.MinItems']) {
        cols.push(
          { ref: [compName], count: true },
          { xpr: stringifyValues(comp['@Validation.MinItems']?.xpr ?? [{ val: comp['@Validation.MinItems'] ?? -1 }]), as: `min${compName}Target` },
          { xpr: stringifyValues(comp['@Validation.MaxItems']?.xpr ?? [{ val: comp['@Validation.MaxItems'] ?? -1 }]), as: `max${compName}Target` }
        )
      }
    }
    return cols;
  ...
  /**
   * Stringifies number values in an expression array
   * @param {*} xpr - expression array
   * @returns {*} - expression array with stringified number values
   */
  ...
  /**
   * Builds a where clause for the keys of the target entity based on the provided data
   * @param {import('@sap/cds').Entity} target - target entity
   * @param {*} data - data object
   * @returns {Array} - where clause array
   */
  ...
  /**
   * Recursively checks compositions for min/max items and raises errors/warnings
   * @param {import('@sap/cds')} target - target entity
   * @param {*} data - data object
   * @param {import('@sap/cds').Request} req - request object
  ...
  function checkCompositionAmounts(target, data, req, path = []) {
    for (const compName in target.compositions) {
      ...
      if (comp._target.compositions && data[compName]) {
        const rows = Array.isArray(data[compName]) ? data[compName] : [data[compName]];
        rows.forEach(row => checkCompositionAmounts(comp._target, row, req, [...path, { id: compName, where: buildKeyWhere(comp._target, row) }]));
      }
      // Only proceed if min/max validation is present
      if (!comp['@Validation.MaxItems'] && !comp['@Validation.MinItems']) continue;

      const amt = data[`${compName}@odata.count`] ?? data[compName]?.length ?? (req.event === 'CREATE' ? 0 : undefined);
      ...
      // Build target path for error messages
      let targetPath = compName;
      if (cds.odata?.urlify) {
        try {
          const { path: url } = cds.odata.urlify(SELECT.from({ ref: path.concat(compName) }), { model: cds.context.model ?? cds.model });
          targetPath = url.substring(url.indexOf('/') + 1);
        } catch { /* fallback to compName */ }
      }
      ...
      if (amt !== undefined && minTarget >= 0 && Number(amt) < Number(minTarget)) {
        ...
        req.target.isDraft ? req.warn(message) : req.error(message);
      }
      // Max items check
    ...
    if (req.query.INSERT?.into.ref.length > 1) {
      ...
      if (comp['@Validation.MaxItems']) {
        ...
        if (typeof comp['@Validation.MaxItems'] === 'number') {
          ...
          parentQuery.SELECT.columns = [{ func: 'count', args: [{ ref: [compName] }], as: 'amt' }, { xpr: stringifyValues(comp['@Validation.MaxItems'].xpr), as: 'target' }]
  ...
  this.before(['DELETE', 'CANCEL'], async req => {
    if (req.query.DELETE.from.ref.length > 1) {
      ...
      if (typeof comp['@Validation.MinItems'] === 'number') {
        ...
        parentQuery.SELECT.columns = [{ func: 'count', args: [{ ref: [compName] }], as: 'amt' }, { xpr: stringifyValues(comp['@Validation.MinItems'].xpr), as: 'target' }]
        const { amt, target } = await cds.run(parentQuery);
        amount = amt;
        targetAmount = target;
      }

...
cds.once('served', async function PluginHandlers () {
  ...
  async function DeleteAttachmentsAfterUpdate(upddatedRecord, data) {
  ...
  function DraftSaveHandler4(composition) {
    ...
    const queryFields = getElements(Attachments);
    return async (_, req) => {
  ...
  function getElements (Attachments) {
    const attachmentFields = ['filename','mimeType','content'];
    let { up_ } = Attachments.keys;
    if (up_) return up_.keys.map(k => 'up__'+k.ref[0]).concat(...attachmentFields).map((k)=>({ref:[k]}));

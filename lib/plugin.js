...
cds.once("served", () => {
  ...
  db.prepend(() => {
    db.on("INSERT", async function handleDBInsert(req, next) {
      ...
      if (
        !req.data ||
        (!req?.target?._attachments?.isAttachmentsEntity && !req?.target?._attachments?.hasInlineAttachments) ||
      ...
      const hasContent = entries.some((entry) => entry.content || prefixes.some((prefix) => entry[`${prefix}_content`]))
      if (!hasContent) {
        return next()
      }
...
cds.ApplicationService.handle_attachments = cds.service.impl(async function () {
  ...
  this.prepend(() =>
  this.on(["PUT", "UPDATE"], async function putUpdateInlineAttachments(req, next) {
    let attachmentTarget
    let prefix

    // Scenario 1: Direct PUT on a stream property (e.g., .../myAttachment_content)
    // req.subject tells us the specific property being updated.
    if (req.subject?.ref?.length > 1 && typeof req.subject.ref[1] === 'string') {
      const parentEntityName = req.subject.ref[0].id
      const streamPropertyName = req.subject.ref[1]
      
      // Get the full, original entity definition from the model
      const parentEntityDefinition = this.model.definitions[parentEntityName]
      if (!parentEntityDefinition?._attachments?.hasInlineAttachments) return next()
      
      // Find the prefix by matching the stream property name
      prefix = parentEntityDefinition._attachments.inlineAttachmentPrefixes.find(p => `${p}_content` === streamPropertyName)
      
      if (prefix) {
        // On the original definition, the compositions object is correctly populated.
        // This is the key part that was missing.
        attachmentTarget = parentEntityDefinition.compositions[prefix]?._target
      }

    } else { 
      // Scenario 2: Deep CREATE/UPDATE where content is provided inline
      if (!req.target._attachments.hasInlineAttachments) return next()
      prefix = req.target._attachments.inlineAttachmentPrefixes.find(p => req.data[`${p}_content`])
      if (prefix) {
        // In this case, req.target is the parent entity, and we can find the composition in its elements.
        attachmentTarget = req.target.elements[prefix]?._target
      }
    }

    // If we couldn't determine the target or prefix, it's not for us.
    if (!prefix || !attachmentTarget) return next()

    // --- The rest of the handler logic ---

    // Create a temporary request object for validation, as the validators expect data on req.data
    const validationReq = { data: { content: req.data[`${prefix}_content`] }, target: attachmentTarget, user: req.user }
    if (!validateAttachmentMimeType(validationReq)) return
    if (!(await validateAttachmentSize(validationReq))) return

    // onPrepareAttachment modifies req.data directly, adding filename, etc.
    await onPrepareAttachment(req) 
    
    if (getAttachmentKind() !== "db") {
      // Object store: upload content to store, metadata written by next()
      // TODO: implement proper inline object store put

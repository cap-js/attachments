...
cds.once("served", async function registerPluginHandlers () {
      ...
      Object.values(srv.entities).forEach((entity) => {
        ...
        for (let elementName in entity.elements) {
          ...
          // For non draft mode
            ...
            srv.before("CREATE", target, (req) => validateAttachmentSize(req))
            ...
            srv.after('CREATE', target, (data) => putAttachment(data,target));
          }
        }
      })
    }
  ...
  async function putAttachment(data,target){
    await AttachmentsSrv.put(target, data); // Insert data into the database for non draft mode
  }
})

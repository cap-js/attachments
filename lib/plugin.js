  ...
  this.before("CREATE", async (req) => {
    ...
    for (const entry of entries) {
      if (entry.attachments) {
        const attachmentTarget = req.target.elements.attachments._target
        const existingAttachments = await SELECT.from(attachmentTarget).where({
          up__ID: entry.ID,
        })
        const filenamesInRequest = new Set(
          existingAttachments.map((att) => att.filename),
        )
        for (const attachment of entry.attachments) {
          attachment.up__ID = entry.ID
          await handleDuplicateFilename(
            attachment,
            attachmentTarget,
            filenamesInRequest,
          )
        ...
        await handleDuplicateFilename(entry, req.target)
      }
    }

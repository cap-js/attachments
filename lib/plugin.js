...
// REVISIT: Once internal issue has fixed in cds-dk #18021
/**
 * Adds i18n folder from attachments plugin to cds environment
 * required for message translations in builded applications
 */
cds.on("served", () => {
  const i18nPluginFolder = path.join(process.cwd(), "node_modules/@cap-js/attachments/_i18n")
  if (fs.existsSync(i18nPluginFolder)) {
    cds.env.i18n.folders.push(i18nPluginFolder)
    LOG.debug(`Added i18n folder from attachments plugin: ${i18nPluginFolder}`)
  }
})
    ...
    for (const compName in target.compositions) {
      ...
      if (comp._target.compositions) {
        if (Array.isArray(data[compName])) {
          data[compName].forEach(
            row => checkCompositionAmounts(comp._target, row, req, [...path, { id: compName, where: buildKeyWhere(comp._target, row) }])
          )

    ...
    if (srv instanceof cds.ApplicationService) {
      Object.values(srv.entities).forEach((entity) => {
        ...
        if (mediaDraftEntities.length) {
          ...
          srv.before(["DELETE", "UPDATE"], entity, AttachmentsSrv.attachDeletionData.bind(AttachmentsSrv))
          srv.after(["DELETE", "UPDATE"], entity, AttachmentsSrv.deleteAttachmentsWithKeys.bind(AttachmentsSrv))

          // case: attachments uploaded in draft and draft is discarded
          srv.before("CANCEL", entity.drafts, AttachmentsSrv.attachDraftDiscardDeletionData.bind(AttachmentsSrv))
          srv.after("CANCEL", entity.drafts, AttachmentsSrv.deleteAttachmentsWithKeys.bind(AttachmentsSrv))
        }
        if (mediaEntities.length) {
          AttachmentsSrv.registerUpdateHandlers(srv, mediaEntities)
          srv.before(["DELETE", "UPDATE"], entity, AttachmentsSrv.attachDeletionData.bind(AttachmentsSrv))
          srv.after(["DELETE", "UPDATE"], entity, AttachmentsSrv.deleteAttachmentsWithKeys.bind(AttachmentsSrv))
        }
      })

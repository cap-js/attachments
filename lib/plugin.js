...
function unfoldModel(csn) {
  ...
  cds.linked(csn).forall("entity", (entity) => {
    const prefixes = entity._attachments?.inlineAttachmentPrefixes
    if (!prefixes?.length) return
    for (const prefix of prefixes) {
      const contentElement = entity.elements?.[`${prefix}_content`]
      if (!contentElement) continue
      if (contentElement["@Core.ContentDisposition.Filename"]) continue // already set
        ...
        if (getAttachmentKind() !== "db") {
          ...
          if (content) {
            ...
            try {
              ...
              const malwareScanner = await cds.connect.to("malwareScanner")
              const scanResult = await malwareScanner.send("scan", {
                file: content,
              })

...
const { inferTargetCAP8 } = require("./helper")
...
cds.ApplicationService.handle_attachments = cds.service.impl(async function () {
  ...
  this.before(['CREATE', 'UPDATE'], async req => {
    ...
    if (req.query.INSERT?.into.ref.length > 1) {
      ...
      const parent = cds.infer?.target?.(parentQuery) || inferTargetCAP8(req, ref)
      if (!parent) {
        LOG.warn(`Could not determine parent target. Ref: ${JSON.stringify(ref)}`)
        return
      }

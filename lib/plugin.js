    ...
    if (srv instanceof cds.ApplicationService) {
      Object.values(srv.entities).forEach((entity) => {
        ...
        for (let elementName in entity.elements) {
          ...
          if (isAttachmentAnnotated(target)) {
            DEBUG?.("Serving attachments for:", target.name)
            const isDraftTarget = !!target?.drafts;
            const targets = isDraftTarget ? [target, target.drafts] : [target];
            ...
            if (isDraftTarget) {
              ...
              AttachmentsSrv.registerDraftUpdateHandlers(srv, entity, target.drafts);
            } else {

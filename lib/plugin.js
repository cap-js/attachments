...
cds.once("served", async function registerPluginHandlers() {
  ...
  cds.model.definitions["sap.common.Images"].isImage = true;
      ...
      Object.values(srv.entities).forEach((entity) => {
        ...
        for (let elementName in entity.elements) {
          ...
          if (target?.["@_is_media_data"]) {
            ...
            const handler = target.isImage ? readImage : readAttachment;
            ...
            srv.before('NEW', entity.drafts, req => {
        ...
        if (mediaElements) srv.after("READ", entity, addImageUrl);
      });
    }
  }

    ...
    this.on(
      ...
      async function putUpdateAttachments(req, next) {
        ...
        if (req.req?.url?.endsWith('/content')) {
          const attachmentID = req.data.ID || req.params?.ID || req.params?.[req.params.length - 1]?.ID
          if (!attachmentID) return req.reject(400, "Missing attachment ID")

          const entityName = req.target.query?.source?.ref?.[0] || req.target.name
          contentExists = await cds.run(
            SELECT.one.from(entityName).where({ ID: attachmentID, content: { '!=': null }})
          )

          if (contentExists) {
            return req.reject(409, "Attachment with given ID already exists and cannot be overwritten")
          }
        }
        ...
        if (contentExists?.url) {
          const exists = await AttachmentsSrv.exists(contentExists.url)
          if (exists) {
            return req.reject(409, "Attachment with given ID already exists and cannot be overwritten")
          }
        }

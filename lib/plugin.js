  ...
  cds.linked(csn).forall("entity", (entity) => {
    ...
    for (const [name, comp] of Object.entries(entity.compositions ?? {})) {
      ...
      if (comp["@attachments.disable_facet"] !== undefined)
        LOG.warn(`@attachments.disable_facet is deprecated! Please annotate ${name} with @UI.Hidden`)
      if (!facets || comp["@attachments.disable_facet"] || hasFacetForComp(name, facets)) continue
      ...
      if (comp["@UI.Hidden"]) attachmentsFacet["@UI.Hidden"] = comp["@UI.Hidden"]
      ...
      Object.keys(comp._target.elements)
        ...
        .forEach((e) => { comp._target.elements[e]["@UI.Hidden"] = true })
      if (comp._target.elements["up_"]) comp._target.elements["up_"]["@UI.Hidden"] = true
    }

    for (const prefix of entity._attachments?.inlineAttachmentPrefixes ?? []) {
      const contentElement = entity.elements?.[`${prefix}_content`]
      if (!contentElement || contentElement["@Core.ContentDisposition.Filename"]) continue
      contentElement["@Core.ContentDisposition.Filename"] = { "=": `${prefix}_filename` }
      ...
      const alreadyConfigured = Object.keys(entity).some(
        (k) =>
          ...
          k.slice("@UI.FieldGroup#".length).toLowerCase() === prefix.toLowerCase(),
      )
      if (!facets || alreadyConfigured || facets.some((f) => f.Target === fieldGroupKey)) continue

      LOG.debug(`Adding @UI.FieldGroup and @UI.Facet for inline attachment to: ${entity.name}`)
      ...
      facets.push({ $Type: "UI.ReferenceFacet", Target: fieldGroupKey, Label: "{i18n>Attachment}" })
    }
  })
...
cds.ApplicationService.handle_attachments = cds.service.impl(async function () {
  ...
  this.before("CREATE", async (req) => {
    if (cds.env.requires.attachments?.deduplicateFileNames === true) {
      ...
      const attachmentCompEntry = Object.entries(
        req.target?.compositions || {},
      ).find(([, comp]) => comp._target?.["@_is_media_data"])
      const attachmentCompName = attachmentCompEntry?.[0]
      const attachmentTarget = attachmentCompEntry?.[1]?._target || req.target
      if (attachmentTarget && attachmentTarget.elements) {

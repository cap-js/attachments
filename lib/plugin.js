  ...
  db.prepend(() => {
    db.on("INSERT", async function handleDBInsert(req, next) {
      ...
      const hasContent = entries.some(
        (entry) => entry.content && entry.content.on,
      )
      if (!hasContent) {
        return next()
      }
  ...
  if (meta.flavor === "inferred") {
    ...
    cds.linked(csn).forall("Composition", (comp) => {
      if (
        ...
        // Add @Common.SideEffects annotation to attachment entity
        const targetDef = csn.definitions[comp._target.name]
        if (targetDef && !targetDef["@Common.SideEffects#attachmentStatusChanged.SourceEvents"]) {
          targetDef["@Common.SideEffects#attachmentStatusChanged.SourceEvents"] = ["attachmentStatusChanged"]
          targetDef["@Common.SideEffects#attachmentStatusChanged.TargetProperties"] = ["status"]
          targetDef["@Common.SideEffects#attachmentStatusChanged.TargetEntities"] = [{ "=": "statusNav" }]
        }

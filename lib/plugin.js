...
function unfoldModel(csn) {
  ...
  cds.linked(csn).forall("entity", (entity) => {
    ...
    for (const [name, comp] of Object.entries(entity.compositions ?? {})) {
      ...
      if (cds.requires?.attachments?.inline === true) {
        comp._target.elements.content["@Core.ContentDisposition.Type"] =
          "inline"
      }
    }
    ...
    for (const prefix of entity._attachments?.inlineAttachmentPrefixes ?? []) {
      ...
      if (contentElement["@UI.Hidden"])
        inlineFacet["@UI.Hidden"] = contentElement["@UI.Hidden"]
      facets.push(inlineFacet)
    }
  })
  ...
  this.prepend(() =>
    this.on(
      ...
      async function putUpdateInlineAttachments(req, next) {
        ...
        if (!validateAttachmentMimeType(req, prefix)) return

...
class AttachmentsService extends cds.Service {
  ...
  registerDraftUpdateHandlers(srv) {
    srv.after("SAVE", async function saveDraftAttachments(res, req) { 
      if (req.target.isDraft || !req.target._attachments.hasAttachmentsComposition || !req.target._attachments.attachmentCompositions) return;
      await Promise.all(Object.keys(req.target._attachments.attachmentCompositions).map(attachmentsEle => this.draftSaveHandler(req.target.elements[attachmentsEle]._target)(res, req)))
    }.bind(this))
  }

...
module.exports = class AttachmentsService extends cds.Service {
  ...
  async put (Attachments, data, _content) {
    ...
    if (!Attachments.elements.filename) { // Skip filenames for sap.common.Images
      if (data.filename) data = _skip_filename(data)
      else data = data.map?.(_skip_filename)
    }
...
const _skip_filename = data => {
  const { filename, ...rest } = data // eslint-disable-line no-unused-vars
  return rest
}

...
const { SELECT, UPSERT, DELETE, UPDATE } = cds.ql;
...
module.exports = class AttachmentsService extends cds.Service {
  ...
  async put(attachments, data, _content) {
    ...
    data.map((d) => { scanRequest(attachments, { ID: d.ID })})
    return res;
  }
  ...
  async delete(Attachments, data) {
    ...
    return await DELETE.from(Attachments).where({ up__ID: data.up__ID, filename: data.filename })
  }
  ...
  async update(Attachments, key, data) {
    ...
    // try {
    //   const entry = await SELECT.from(Attachments, key)
    //   const res = await UPDATE(Attachments, key).with(data)
    //   return res
    // } catch (err) {
    //   DEBUG?.("Error updating attachment", err)
    // }
  }
};

...
async function validateAttachment(req) {
  ...
  if (req?.req?.url?.endsWith("/content")) {
    ...
    if (req.target?._attachments.isAttachmentsEntity) {
      ...
      const record = await SELECT.one.from(req.target, parentKeys).columns(`${prefix}_ID as ID`, `${prefix}_status as status`, `${prefix}_lastScan as lastScan`)
      if (record) {
        status = record.status
        lastScan = record.lastScan
        attachmentId = record.ID
        target = req.target.elements[prefix]._target
        keys = { up__ID: parentKeys.ID, ID: attachmentId } // Assuming parent has ID
      }
    }

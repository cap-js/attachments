...
async function finalizePrepareAttachment(data, req) {
  ...
  data.mimeType = getMime(ext)

  if (!data.mimeType) {
    LOG.warn(
      `An attachment ${data.ID} is uploaded whose extension "${ext}" is not known! Falling back to "application/octet-stream"`,
    )
...
async function validateAttachment(req) {
  ...
  if (req?.req?.url?.endsWith("/content")) {
    ...
    if (scanEnabled) {
      if (status !== "Clean") {
        await AttachmentsSrv.emit("AttachmentDownloadRejected", {
...
async function validateAttachmentSize(req, validateContentLength = false) {
  if (
    !req.target?._attachments.isAttachmentsEntity ||
    (!req.data.content && !validateContentLength)
  )
    return false
  ...
  if (length > maxFileSize) {
    ...
    const AttachmentsSrv = await cds.connect.to("attachments")
    AttachmentsSrv.emit("AttachmentSizeExceeded", {
      target: req.target.name,
      keys: req.data.ID ? { ID: req.data.ID } : {},
      filename: attachmentRef?.filename ?? "n/a",
      fileSize: length,
      maxFileSize,
    }).catch((err) => LOG.error("Failed to emit AttachmentSizeExceeded", err))
 ...
 * @param {import('@sap/cds').Request} req - The request object
 */
async function validateAttachmentMimeType(req) {
  ...
  if (!checkMimeTypeMatch(acceptableMediaTypes, mimeType)) {
    ...
    }).catch((err) => LOG.error("Failed to emit AttachmentUploadRejected", err))
...
async function validateAndInsertAttachmentFromDBHandler(data, target, req) {
  ...
  if (!(await validateAttachmentMimeType({ data, target, reject: req.reject })))
    return
  if (
    !(await validateAttachmentSize({ data, target, reject: req.reject }, true))
  )
    return

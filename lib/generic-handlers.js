...
async function finalizePrepareAttachment(data, req) {
  ...
  if (!hasUpKey) {
    ...
    if (cds.infer?.target) {
      ...
      // CAP 8 fallback: Look up in model.definitions
      const parentEntityName = parentRef.join('.')
      target = cds.context?.model?.definitions?.[parentEntityName] ?? cds.model?.definitions?.[parentEntityName]
    }

    if (!target?.keys) {
      LOG.warn(`Could not determine parent target for attachment upload. ParentRef: ${parentRef.join('.')}`)
      return
    }

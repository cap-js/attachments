...
async function readAttachment([attachment], req) {
  ...
  if (req.target?._attachments?.isAttachmentsEntity) {
    ...
    attachment.content = await AttachmentsSrv.get(req.target, { ID: req.data.ID ?? req.params.at(-1).ID }, null)
  } else {
    ...
    attachment[contentField] = await AttachmentsSrv.get(req.target, parentKeys, record.url)
  }
}

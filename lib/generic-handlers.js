  ...
  if (req?.req?.url?.endsWith("/content")) {
    ...
    if (scanEnabled) {
      ...
      if (scanExpiryMs != -1 && (!lastScan || (Date.now() - new Date(lastScan).getTime() > scanExpiryMs))) {
        ...
        await cds.tx(async () => await malwareScanner.updateStatus(target, keys, "Scanning"))

        // Trigger scan in a separate thread to 
        cds.spawn(async () => {
        ...
        req.reject(202, 'UnableToDownloadAttachmentScanStatusExpired')
      }
    }
  }
}

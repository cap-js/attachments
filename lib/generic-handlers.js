...
async function validateAndInsertAttachmentFromDBHandler(data, target, req) {
  ...
  // Prevent storing the content in the database
  data.url = cds.utils.uuid()

  const attachment = await cds.connect.to("attachments")

  // Pass skipDbInsert flag to indicate the DB INSERT will be handled by CAP's native INSERT
  // that follows this db.before handler
  await attachment.put(target, data, { skipDbInsert: true })

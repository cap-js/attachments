...
async function finalizePrepareAttachment(data, req) {
  ...
  if (!hasUpKey) {
    if (!req.subject?.ref || req.subject.ref.length < 2) return
    ...
    if (cds.infer?.target) {
      // CDS 9+
      parentTarget = cds.infer.target({
        SELECT: { from: { ref: parentRef } }
      })
    } else {
      // CDS 8 fallback
      const last = parentRef[parentRef.length - 1]
      const parentEntity = last.id ?? last
      parentTarget = cds.model.definitions[parentEntity]
    }

    if (!parentTarget?.keys) return

    const parentKeys = Object.keys(parentTarget.keys)

    const parentRecord = await SELECT
      .one
      .from({ ref: parentRef })
      .columns(parentKeys)

 ...
 * @param {import('@sap/cds').Request} req - The request object
 * @param {string} [knownPrefix] - Prefix to use directly, skipping the req.data lookup (stream PUT path)
 */
function validateAttachmentMimeType(req, knownPrefix) {
  if (
    !req.target?._attachments?.isAttachmentsEntity &&
    !req.target?._attachments?.hasInlineAttachments
  )
  ...
  if (req.target?._attachments?.isAttachmentsEntity) {
    ...
    const prefix =
      knownPrefix ??
      req.target?._attachments?.inlineAttachmentPrefixes.find(
        (p) =>
          req.data[`${p}_content`] ||
          req.headers["content-disposition"]?.includes(`name="${p}_content"`),
      )

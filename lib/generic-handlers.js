...
async function finalizePrepareAttachment(data, req, prefix) {
  ...
  const hasUpKey = Object.keys(attachmentData).some((key) => key.startsWith("up__"))
  ...
  let ext = attachmentData.filename ? extname(attachmentData.filename).toLowerCase().slice(1) : null
...
async function validateAttachment(req) {
  if (!req.target?._attachments.isAttachmentsEntity && !req.target?._attachments.hasInlineAttachments) return
  ...
  const isContentRequest = reqUrl?.endsWith("/content") || /\/[^/]*_content$/.test(reqUrl)
  ...
  if (isContentRequest) {
    ...
    if (req.target?._attachments.isAttachmentsEntity) {
      ...
      const { IsActiveEntity, ...dbKeys } = Object.fromEntries(
        Object.entries(req.target.keys)
          .map(([key, value]) => [key, req.data[key] || req.params.find(p => p[key])?.[key]])
          .filter(([, value]) => value !== undefined)
      )

      const parentRecord = await SELECT.one.from(req.target, dbKeys).columns(`${prefix}_status`, `${prefix}_lastScan`)
    ...
    if (scanEnabled) {
      ...
      if (
        ...
        if (req.target?._attachments.isAttachmentsEntity) {
          ...
          const prefix = reqUrl.split('/').pop().replace('_content', '')
          cds.tx(async () => await UPDATE(req.target, keys).set({ [`${prefix}_status`]: "Scanning" }))
          cds.spawn(async () => await malwareScanner.emit("ScanAttachmentsFile", { target: req.target.name, keys }))
        }
...
async function readAttachment([attachment], req) {
  ...
  if (req.target?._attachments.isAttachmentsEntity) {
    ...
    const record = await SELECT.one.from(req.target, parentKeys).columns(`${prefix}_ID as ID`)
    if (!record?.ID) return
    keys = { ID: record.ID }
...
async function validateAttachmentSize(req, validateContentLength = false) {
  if (!req.target?._attachments.isAttachmentsEntity && !req.target?._attachments.hasInlineAttachments)
    return false
  ...
  if (req.target?._attachments.isAttachmentsEntity) {
    ...
    prefix = req.target?._attachments.inlineAttachmentPrefixes.find(p => req.data[`${p}_content`] || req.headers['content-disposition']?.includes(`name="${p}_content"`))
  ...
  if (!validateContentLength && !isInMemory) {
    if (
      ...
      if (content?.pipe) return true 
      req.reject(411, "ContentLengthHeaderMissing")
      return false
      // return true
    }
  ...
  const length = (validateContentLength || isInMemory)
    ? content.length
  ...
  if (length > maxFileSize) {
    ...
    if (req.target?._attachments.isAttachmentsEntity) {
      ...
      const attachmentRef = attachmentId
        ? await SELECT.one("filename").from(req.target).where({ ID: attachmentId })
        : null
      filename = req.data.filename ?? attachmentRef?.filename ?? "n/a"
    } else {
      filename = req.data[`${prefix}_filename`] ?? null
      if (!filename) {
        const { IsActiveEntity, ...dbKeys } = req.params?.at(-1) || {}
        const record = await SELECT.one.from(req.target.drafts || req.target, dbKeys).columns(`${prefix}_filename`)
    ...
    req.reject({
      ...
      args: [
        filename,
        element["@Validation.Maximum"] ?? "400MB",
      ],
    })
    return false
  }
...
function validateAttachmentMimeType(req) {
  ...
  if (req.target?._attachments.isAttachmentsEntity) {
    ...
    const prefix = req.target?._attachments.inlineAttachmentPrefixes.find(p => req.data[`${p}_content`] || req.headers['content-disposition']?.includes(`name="${p}_content"`))
    if (!prefix) return false
  ...
  const acceptableMediaTypes =
    element["@Core.AcceptableMediaTypes"] || "*/*"
...
async function validateAndInsertAttachmentFromDBHandler(data, target, req) {
  ...
  if (target._attachments.isAttachmentsEntity) {
    ...
    if (!validateAttachmentMimeType({ data, target, reject: req.reject })) return
    if (!(await validateAttachmentSize({ data, target, reject: req.reject }, true))) return
    ...
    for (const prefix of target._attachments.inlineAttachmentPrefixes) {
      if (data[`${prefix}_content`]) {
        ...
        const attachmentTarget = cds.model.definitions["sap.attachments.Attachment"]
        if (!validateAttachmentMimeType({ data: attachmentData, target: attachmentTarget, reject: req.reject })) continue
        if (!(await validateAttachmentSize({ data: attachmentData, target: attachmentTarget, reject: req.reject }, true))) continue

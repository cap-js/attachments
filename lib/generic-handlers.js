...
function validateAttachmentSize(req) {
  ...
  const maxFileSize = sizeInBytes(req.target.elements['content']['@Validation.Maximum'], req.target.name) ?? MAX_FILE_SIZE;

  if (req.headers["content-length"] && Number(req.headers["content-length"]) > maxFileSize) {
    req.data.content.pause()
    return req.reject(413, "AttachmentSizeExceeded")
  }
}

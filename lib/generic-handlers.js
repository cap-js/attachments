...
async function finalizePrepareAttachment(data, req, prefix) {
  ...
  if (!prefix) attachmentData.status ??= "Unscanned"
...
async function validateAttachment(req) {
    ...
    for (const prefix of req.target._attachments.inlineAttachmentPrefixes) {
      req?.query?.SELECT?.columns?.forEach((element) => {
        if (
          ...
          // The xpr checks content IS NOT NULL, but content is always null in hybrid/object-store
          // mode (files stored externally). Swap the content ref to url: url is set only when a
          // real file exists, so this correctly returns null for new entities with no upload.
  ...
  if (isContentRequest) {
    ...
    let status, lastScan, attachmentId, inlineUrl
    ...
    if (req.target?._attachments?.isAttachmentsEntity) {
      ...
      inlineUrl = parentRecord[`${prefix}_url`]
    }
    ...
    if (scanEnabled) {
      ...
      const prefix = req.target?._attachments?.isAttachmentsEntity
        ? undefined
        : reqUrl.split("/").pop().replace("_content", "")

...
const _createObjectStoreInstance = async (sm_url, tenant, offeringName, planID, token) => {
    ...
    // Validate inputs
    if (!sm_url || !tenant || !offeringName || !planID || !token) {
    ...
    if (profile === 'mtx-sidecar') {
        ...
        ds.after('subscribe', async (_, req) => {
            ...
            try {
                ...
                // Create Instance
                const instancePath = await _createObjectStoreInstance(sm_url, tenant, LABEL.OBJECT_STORE, planID, token);

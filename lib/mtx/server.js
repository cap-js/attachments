 ...
 */
const _serviceManagerRequest = async (
  sm_url,
  method,
  path,
  token,
  ...
  try {
    const queryString = Object.keys(params).length
      ...
      : ""
    const { data } = await _fetch(`${sm_url}/${path}${queryString}`, {
    ...
    LOG.error(
      `Service Manager API request failed - ${method.toUpperCase()} ${path}`,
      error,
...
const _fetchToken = async (url, clientid, clientsecret, certificate, key) => {
  try {
    ...
    if (clientid && clientsecret) {
      LOG.debug("Using OAuth client credentials to fetch token.", {
        url,
        clientid,
      })
      ...
      if (!tokenData?.access_token) {
        LOG.error(
          ...
          { clientid, responseData: tokenData },
        )
    ...
    LOG.debug(
      "OAuth client credentials missing - checking for MTLS credentials",
      { url, clientid },
    )
    ...
    if (certificate && key) {
      ...
      const { data: mtlsData } = await _fetchWithAgent(tokenUrl, {
        method: "POST",
        headers,
        body,
        agent,
      ...
      if (!mtlsData?.access_token) {
        LOG.error(
          "MTLS token response missing access_token",
          null,
          "Check MTLS certificate/key validity and Service Manager configuration",
          { clientid, responseData: mtlsData },
        )
        throw new Error("Access token not found in MTLS token response")
      }

      return mtlsData.access_token
    }

    // If neither flow is possible
    throw new Error(
      "Missing authentication credentials: Provide either OAuth clientid/clientsecret or MTLS certificate/key.",
    )
  } catch (error) {
    LOG.error(
      "Failed to fetch OAuth token using provided credentials",
      error,
...
const _getOfferingID = async (sm_url, token) => {
  const offerings = await _serviceManagerRequest(
    sm_url,
    HTTP_METHOD.GET,
    PATH.SERVICE_OFFERING,
    token,
    { fieldQuery: "name eq 'objectstore'" },
  )
  const offeringID = offerings.id
  if (!offeringID)
    LOG.debug("Object store service offering not found in Service Manager", {
      sm_url,
    })
  return offeringID
}
...
const _getPlanID = async (sm_url, token, offeringID) => {
  ...
  for (const planName of supportedPlans) {
    ...
    try {
      ...
      if (plan?.id) {
        LOG.debug("Using object store plan", {
          planName,
          planID: plan.id,
          offeringID,
        })
      ...
      LOG.error(
        `Failed to fetch plan "${planName}" from Service Manager`,
        error,
        "Check Service Manager connectivity and credentials",
  ...
  LOG.debug("No supported object store service plan found in Service Manager", {
    sm_url,
    attempted: supportedPlans.join(", "),
  })
  throw new Error(
...
const _createObjectStoreInstance = async (sm_url, tenant, planID, token) => {
  try {
    const { headers } = await _fetch(`${sm_url}/v1/service_instances`, {
      method: "POST",
      headers: {
        Accept: "application/json",
        Authorization: `Bearer ${token}`,
    ...
    LOG.error(
      `Failed to create object store instance for tenant - ${tenant}`,
      error,
      "Check Service Manager connectivity and credentials",
  ...
  try {
    ...
    while (!isReady) {
      ...
      if (Date.now() - startTime > ASYNC_TIMEOUT) {
        LOG.debug("Timed out waiting for service instance to be ready", {
          instancePath,
          sm_url,
        })
      }
      ...
      if (instanceStatus.data.state === STATE.FAILED) {
        LOG.debug("Service instance creation failed", {
          instancePath,
          sm_url,
          details: instanceStatus.data,
        })
      }
    }
  } catch (error) {
    LOG.error(
      "Error polling for object store instance readiness",
      error,
...
const _bindObjectStoreInstance = async (sm_url, tenant, instanceID, token) => {
  if (instanceID) {
    try {
      const { data } = await _fetch(`${sm_url}/${PATH.SERVICE_BINDING}`, {
        method: "POST",
        headers: {
          Accept: "application/json",
          Authorization: `Bearer ${token}`,
      ...
      LOG.error(
        `Error binding object store instance for tenant - ${tenant}`,
        error,
        "Check Service Manager connectivity and credentials",
...
const _getBindingIdForDeletion = async (sm_url, tenant, token) => {
  try {
    const getBindingCredentials = await _serviceManagerRequest(
      sm_url,
      HTTP_METHOD.GET,
    ...
    if (!getBindingCredentials?.id) {
      LOG.warn("No binding credentials found for tenant during deletion", {
        tenant,
      })
    ...
    LOG.error(
      `Error fetching binding credentials for tenant - ${tenant}`,
      error,
      "Check Service Manager connectivity and credentials",
      { sm_url, tenant },
    )
...
const _deleteBinding = async (sm_url, bindingID, token) => {
  if (bindingID) {
    try {
      await _fetch(`${sm_url}/${PATH.SERVICE_BINDING}/${bindingID}`, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
          Authorization: `Bearer ${token}`,
      ...
      LOG.error(
        `Error deleting binding - ${bindingID}`,
        error,
        "Check Service Manager connectivity and credentials",
        { sm_url, bindingID },
      )
...
const _getInstanceIdForDeletion = async (sm_url, tenant, token) => {
  try {
    const instanceId = await _serviceManagerRequest(
      sm_url,
      HTTP_METHOD.GET,
    ...
    LOG.error(
      `Error fetching service instance id for tenant - ${tenant}`,
      error,
      "Check Service Manager connectivity and credentials",
      { sm_url, tenant },
    )
...
const _deleteObjectStoreInstance = async (sm_url, instanceID, token) => {
  if (instanceID) {
    try {
      const { headers } = await _fetch(
        `${sm_url}/${PATH.SERVICE_INSTANCE}/${instanceID}`,
        {
          method: "DELETE",
          headers: {
            Accept: "application/json",
      ...
      LOG.error(
        `Error deleting object store instance - ${instanceID}`,
        error,
        "Check Service Manager connectivity and credentials",
        { sm_url, instanceID },
      )
    ...
    if (objectStoreKind === "separate") {
      ds.after("subscribe", async (_, req) => {
        ...
        try {
          const serviceManagerCredentials =
            cds.env.requires?.serviceManager?.credentials || {}
          validateServiceManagerCredentials(serviceManagerCredentials)
          const { sm_url, url, clientid, clientsecret, certificate, key } =
          ...
          const existingTenantBindings = await fetchObjectStoreBinding(
            tenant,
            token,
          )
          ...
          if (existingTenantBindings.length) {
            LOG.info(
              `Existing tenant specific object store for ${tenant} exists. Skipping creation of new one.`,
            )
            return
          }
          ...
          const instanceID = await _createObjectStoreInstance(
            sm_url,
            tenant,
            planID,
            token,
          ...
          LOG.error(
            `Error setting up object store for tenant - ${tenant}`,
            error,
            "Check Service Manager connectivity and credentials",
            { tenant },
          )
      ...
      ds.after("unsubscribe", async (_, req) => {
        ...
        try {
          ...
          const bindingID = await _getBindingIdForDeletion(
            sm_url,
            tenant,
            token,
          )
          ...
          const service_instance_id = await _getInstanceIdForDeletion(
            sm_url,
            tenant,
            token,
          )
          ...
          LOG.error(
            `Error deleting object store service for tenant - ${tenant}`,
            error,
            "Check Service Manager connectivity and credentials",
            { tenant },
          )
      ...
      ds.after("unsubscribe", async (_, req) => {
        ...
        if (!creds)
          throw new Error("SAP Object Store instance credentials not found.")
        ...
        switch (cds.env.requires?.attachments?.kind) {
          ...
          case "gcp":
            ...
            break
          default:
            LOG.warn("Unsupported object store kind for cleanup", {
...
const _cleanupAWSS3Objects = async (creds, tenant) => {
  ...
  try {
    ...
    if (keysToDelete.length > 0) {
      ...
      LOG.debug("[AWS S3] S3 objects deleted for tenant", {
        tenant,
        deletedCount: keysToDelete.length,
      })
    } else {
      LOG.debug("[AWS S3] No S3 objects found for tenant during cleanup", {
        tenant,
      })
    }
  } catch (error) {
    LOG.error(
      `Failed to clean up S3 objects for tenant "${tenant}"`,
      error,
...
const _cleanupAzureBlobObjects = async (creds, tenant) => {
  ...
  try {
    ...
    LOG.debug("[Azure] Azure Blob objects deleted for tenant", {
      tenant,
      deletedCount: blobsToDelete.length,
    })
  } catch (error) {
    LOG.error(
      `Failed to clean up Azure Blob objects for tenant "${tenant}"`,
      error,
...
const _cleanupGoogleCloudObjects = async (creds, tenant) => {
  ...
  try {
    ...
    LOG.debug("[GCP] Google Cloud Storage objects deleted for tenant", {
      tenant,
      deletedCount: totalDeleted,
    })
  } catch (error) {
    LOG.error(
      `Failed to clean up Google Cloud Platform objects for tenant "${tenant}"`,
      error,

    ...
    if (profile === 'mtx-sidecar') {
        ...
        ds.after('subscribe', async (_, req) => {
            ...
            try {
                ...
                const { offeringName, planID } = await _getOfferingAndPlanID(sm_url, token);

                // Create Instance
                const instancePath = await _createObjectStoreInstance(sm_url, tenant, offeringName, planID, token);

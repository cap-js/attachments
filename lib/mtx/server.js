            ...
            ds.after('unsubscribe', async (_, req) => {
                ...
                try {
                    ...
                    for await (const page of paginator) {
                        if (page.Contents) {
                            page.Contents.map((url) => (keysToDelete.push({ Key: url.Key })));
                        }
                    }
                    ...
                    if (keysToDelete.length > 0) {
                        try {
                            ...
                            DEBUG?.("S3 objects deleted for tenant: ", tenant);
                        } catch (error) {
                            // eslint-disable-next-line no-console
                    ...
                    // eslint-disable-next-line no-console
                    console.error(`Error listing objects from S3 bucket: ${error.message}`);
                }
            })
        }
    }

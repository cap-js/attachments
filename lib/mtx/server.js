...
const _fetchToken = async (url, clientid, clientsecret, certificate, key) => {
  try {
    if (clientid && clientsecret) {
      ...
      return (
        await axios.post(`${url}/oauth/token`, null, {
          headers: {
            Accept: "application/json",
    ...
    if (!clientid || !clientsecret) {
      DEBUG?.(
        "Client credentials missing, falling back to MTLS if certificate/key available."
      );
    }

    if (certificate && key) {
      // Use MTLS client certificate flow
    ...
    DEBUG?.("Both client credentials and MTLS certificates are missing.");
    throw new Error(

    ...
    if (profile === 'mtx-sidecar') {
        ...
        if (separateObjectStore === "separate") {
            ...
            ds.after('unsubscribe', async (_, req) => {
                ...
                console.log('Object Store credentilas',this?.options);
                console.log('Environmnet variables',cds.env.requires);
                console.log('Attachmenst crdentials',cds.env.requires?.attachments?.objectstore?.credentials);
                console.log('',cds.env.requires?.objectstore);
                const creds = cds.env.requires?.['attachments-s3']?.credentials;
                console.log("consoling the credentials, required for deleting the shared objects in asw s3",creds);

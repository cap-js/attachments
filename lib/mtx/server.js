...
const _getOfferingID = async (sm_url, token) => {
    ...
    if (!offeringID) DEBUG?.('Service offering not found');
    return offeringID;
}
...
const _getPlanID = async (sm_url, token, offeringID) => {
    ...
    if (!planID) DEBUG?.('Service plan not found');
    return planID;
};
    ...
    if (profile === 'mtx-sidecar' && separateObjectStore === "separate") {
        ...
        ds.after('subscribe', async (_, req) => {
            ...
            try {
                ...
                DEBUG?.(`Error setting up object store for tenant - ${tenant}: ${error.message}`);
            }
        });
        ...
        ds.after('unsubscribe', async (_, req) => {
            ...
            try {
                ...
                DEBUG?.(`Error deleting object store service for tenant - ${tenant}: ${error.message}`);
            }

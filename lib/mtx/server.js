...
const _cleanupGoogleCloudObjects = async (creds, tenant) => {
    ...
    try {
        ...
        logConfig.debug('[GCP] Google Cloud Storage objects deleted for tenant', { tenant, deletedCount: files.length })
    } catch (error) {
        logConfig.withSuggestion('error',
            `Failed to clean up Google Cloud Platform objects for tenant "${tenant}"`, error,
            'Check Google Clous Storage connectivity and permissions',
            { tenant })
    }
}

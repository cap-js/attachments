...
const _serviceManagerRequest = async (sm_url, method, path, token, params = {}) => {
    try {
        ...
        console.error(`Error fetching data from service manager: ${error.message}`);
    }
};
...
const _fetchToken = async (url, clientid, clientsecret) => {
    try {
        ...
        // eslint-disable-next-line no-console
        console.error(`Error fetching token from service manager: ${error.message}`);
    }
};
...
const _getOfferingID = async (sm_url, token) => {
    ...
    // eslint-disable-next-line no-console
    if (!offeringID) console.error('Service offering not found');
    return offeringID;
}
...
const _getPlanID = async (sm_url, token, offeringID) => {
    ...
    // eslint-disable-next-line no-console
    if (!planID) console.error('Service plan not found');
    return planID;
};
...
const _createObjectStoreInstance = async (sm_url, tenant, planID, token) => {
    try {
        ...
        // eslint-disable-next-line no-console
        console.error(`Error creating object store instance - ${tenant}: ${error.message}`);
    }
};
    ...
    try {
        ...
        while (!isReady) {
            ...
            if (Date.now() - startTime > ASYNC_TIMEOUT) {
                // eslint-disable-next-line no-console
                console.error('Timed out waiting for service instance to be ready');
            }

            if (instanceStatus.data.state === STATE.FAILED) {
                // eslint-disable-next-line no-console
                console.error('Service instance creation failed');
            }
        }
    } catch (error) {
        // eslint-disable-next-line no-console
        console.error(`Error polling for object store instance readiness: ${error.message}`);
    }
};
...
const _bindObjectStoreInstance = async (sm_url, tenant, instanceID, token) => {
    if (instanceID) {
        try {
            ...
            // eslint-disable-next-line no-console
            console.error(`Error binding object store instance for tenant - ${tenant}: ${error.message}`);
        }
    }
};
...
const _getBindingIdForDeletion = async (sm_url, tenant, token) => {
    try {
        ...
        // eslint-disable-next-line no-console
        console.error(`Error fetching binding credentials for tenant - ${tenant}: ${error.message}`);
    }
};
...
const _deleteBinding = async (sm_url, bindingID, token) => {
    if (bindingID) {
        try {
            ...
            // eslint-disable-next-line no-console
            console.error('Error deleting binding:', error.message);
        }
    } else {
        // eslint-disable-next-line no-console
...
const _getInstanceIdForDeletion = async (sm_url, tenant, token) => {
    try {
        ...
        // eslint-disable-next-line no-console
        console.error(`Error fetching service instance id for tenant - ${tenant}: ${error, message}`);
    }
}
...
const _deleteObjectStoreInstance = async (sm_url, instanceID, token) => {
    if (instanceID) {
        try {
            ...
            // eslint-disable-next-line no-console
            console.error(`Error deleting object store instance - ${instanceID}: ${error.message}`);
        }
    }
};
    ...
    if (profile === 'mtx-sidecar' && separateObjectStore === "separate") {
        ...
        ds.after('subscribe', async (_, req) => {
            ...
            try {
                ...
                const instanceID = await _createObjectStoreInstance(sm_url, tenant, planID, token); // change var to instanceID

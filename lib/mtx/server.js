...
const _getBindingIdForTenant = async (sm_url, tenant, token) => {
    ...
    try {
        const getBindingCredentials = await _serviceManagerRequest(sm_url, HTTP_METHOD.GET, PATH.SERVICE_BINDING, token, { label: `tenant_id eq '${tenant}'` });
...
const _getServiceInstanceId = async (sm_url, tenant, token) => {
    try {
        const instanceId = await _serviceManagerRequest(sm_url, HTTP_METHOD.GET, PATH.SERVICE_INSTANCE, token, { label: `tenant_id eq '${tenant}'` });

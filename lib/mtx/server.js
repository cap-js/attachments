...
const _getBindingIdForTenant = async(sm_url,tenant, token)=>{
        ...
        try{
            let getBindingCredentials = await axios.get(`${sm_url}/v1/service_bindings`, {
                headers: {
                    'Accept': 'application/json',
                    'Authorization': `Bearer ${token}`
                },
                params: {
                    labelQuery: `tenant_id eq'${tenant}'`
                }
            }); 
            console.log("Binding credentials",getBindingCredentials.data[0].ID);
            return getBindingCredentials.data[0].ID;
        }catch (error) {
...
const _getServiceInstanceId = async (sm_url, tenant, token) => {
    try {
        const instanceId = await axios.get(`${sm_url}/${PATH.SERVICE_INSTANCE}`, {
            headers: {
                'Accept': 'application/json',
                'Authorization': `Bearer ${token}`
            },
        ...
        console.error('Error fetching the instance :', error);
        throw error;
    }
}

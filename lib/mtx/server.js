...
const _serviceManagerRequest = async (sm_url, method, path, token, params = {}) => {
    try {
        ...
        if (!response.data || !response.data.items || response.data.items.length === 0) {
            console.error(`No items found in response from ${path}`);
            return null; // Return null to avoid crashes
        }

        console.log("First item in response:", response.data.items[0]);
...
const _getBindingIdForTenant = async (sm_url, tenant, token) => {
    console.log("inside getting binding id function");
    try {
        ...
        console.log("Raw Binding Response:", getBindingCredentials); // Debugging log
        ...
        console.error("Error fetching instance binding information:", error);
        throw error;
    }
};
...
const _deleteObjectStoreInstance = async (sm_url, instanceID, token) => {
    if (instanceID) {
        try {
            console.log('Instance ID with which the insatnce will be deleted', instanceID)
        ...
        console.log("Instance id is either undefined or null");
    }
};

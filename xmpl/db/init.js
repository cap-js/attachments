      ...
      ds.after("subscribe", async (_, req) => {
        req.on("succeeded", async () => {
          ...
          await cds.tx({ tenant }, async (tx) => {
            ...
            const {
              "sap.capire.incidents.Incidents.attachments": Attachments,
            } = cds.entities;
            await attachments.put(
              Attachments,
              [
                [
                  "3b23bb4b-4ac7-4a24-ac02-aa10cabd842c",
                  "INVERTER FAULT REPORT.pdf",
                ],
                [
                  "3b23bb4b-4ac7-4a24-ac02-aa10cabd842c",
                  "Inverter-error-logs.txt",
                ],
                ["3a4ede72-244a-4f5f-8efa-b17e032d01ee", "No_Current.xlsx"],
                ["3ccf474c-3881-44b7-99fb-59a2a4668418", "strange-noise.csv"],
                [
                  "3583f982-d7df-4aad-ab26-301d4a157cd7",
                  "Broken Solar Panel.jpg",
                ],
              ].map(([up__ID, filename]) => ({
                up__ID,
                filename,
            ...
            const { "sap.capire.incidents.Customers": Customers } =
              cds.entities;
